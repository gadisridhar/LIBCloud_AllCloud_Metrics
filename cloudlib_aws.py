...
from libcloud.compute.constants import ec2_region_details_complete
...
# {
#     "_id" : "0a2eca7ad4a64ebe81a0317387bedc0f",
#     "_cls" : "PollingSchedule.OwnerPollingSchedule.MeteringPollingSchedule",
#     "name" : "push_metering_info(4b42460a9e694313a021d289c1cd05fa)",
#     "default_interval" : {
#         "name" : "default",
#         "every" : 1800
#     },
#     "override_intervals" : [],
#     "owner" : "4b42460a9e694313a021d289c1cd05fa",
#     "shard_id" : "0c9873781e9a",
#     "shard_update_at" : ISODate("2021-10-29T12:01:04.575Z")
# }



def loadaws_polling_data(datahex, cloudPayload, cfgDB):
    message = {}
    try:
        extra = {}
        ...
        message[Names.RESPONSE_MESSAGE] = "AWS Account Pool Session Created"
    except Exception as e:
        print("exception: " + str(e))
        message[Names.RESPONSE_MESSAGE] = str(e)
    return message

def loadaws_subnet_data(datahex, cloudPayload, cfgDB):
    message = {}
    try:
        print("AWS CLOUD SUBNET DETAILS")
        # logger.info("----------  LOADING TENANT CREDENTIALS  ----------")
        #--------------------------------------------DRIVER DETAILS FOR AWS ACCOUNT
        cls = get_compute_driver(ComputeProvider.EC2)
        driver = cls(str(cloudPayload[Names.AWS_APIKEY]), str(cloudPayload[Names.AWS_APISECRET]), region=str(cloudPayload[Names.AWS_REGION]))
        ...
        message[Names.RESPONSE_MESSAGE] = "AWS Subnet Details Added"
    except Exception as e:
        print("exception: " + str(e))
        message[Names.RESPONSE_MESSAGE] = str(e)
    return message
...
def loadaws_network_data(datahex, cloudPayload, cfgDB):
    message = {}
    try:
        print("AWS CLOUD SUBNET DETAILS")
        # logger.info("----------  LOADING TENANT CREDENTIALS  ----------")
        # --------------------------------------------DRIVER DETAILS FOR AWS ACCOUNT
        cls = get_compute_driver(ComputeProvider.EC2)
        driver = cls(str(cloudPayload[Names.AWS_APIKEY]), str(cloudPayload[Names.AWS_APISECRET]),
                     region=str(cloudPayload[Names.AWS_REGION]))
        nodeDetails = driver.list_nodes()
        ...
        message[Names.RESPONSE_MESSAGE] = "AWS Network Details Added"
    except Exception as e:
        print("exception: " + str(e))
        message[Names.RESPONSE_MESSAGE] = str(e)
    return message
...
def loadaws_compute_data(datahex, cloudPayload, cfgDB):
    message = {}
    try:
        # logger.info("----------  LOADING TENANT CREDENTIALS  ----------")
        #--------------------------------------------DRIVER DETAILS FOR AWS ACCOUNT
        ...
        if (len(nodeDetails) > 0):
            for node in nodeDetails:
                ...
                extra[Names.MACHINE_ID] = node.id
                extra[Names.MACHINE_OWNER] = node.name
                extra[Names.MACHINE_STATE] = node.state
            ...
            message[Names.RESPONSE_MESSAGE] = "AWS Compute Details Added"
        else:
            print("empty")
            message[Names.RESPONSE_MESSAGE] = "AWS No Records for Compute"
...
def loadaws_images_data(datahex, cloudPayload,cfgDB):
    message = {}
    try:
        print("AWS CLOUD IMAGE DETAILS")
        # logger.info("----------  LOADING TENANT CREDENTIALS  ----------")
        # --------------------------------------------DRIVER DETAILS FOR AWS ACCOUNT
        cls = get_compute_driver(ComputeProvider.EC2)
        driver = cls(str(cloudPayload[Names.AWS_APIKEY]), str(cloudPayload[Names.AWS_APISECRET]),
        ...
        message[Names.RESPONSE_MESSAGE] = "AWS Image Details Added"
    except Exception as e:
           print("exception: " + str(e))
           message[Names.RESPONSE_MESSAGE] = str(e)
...
def loadaws_location_data(datahex, cloudPayload, configfile):
    message = {}
    try:
        # --------------------------------------------DRIVER DETAILS FOR AWS ACCOUNT
        cls = get_compute_driver(ComputeProvider.EC2)
        ...
        for location in locDetails:
            ...
            extra[Names.AVAILABLITYZONE_NAME] = location.availability_zone.name
            extra[Names.AVAILABLITYZONE_REGION] = location.availability_zone.region_name
            extra[Names.AVAILABLITYZONE_STATE] = location.availability_zone.zone_state
        ...
        #print("---------", locationList)
        DataManager.location_Data(locationList, configfile)

        message[Names.RESPONSE_MESSAGE] = "AWS Location Details Added"
    except Exception as e:
        print("exception: " + str(e))
        message[Names.RESPONSE_MESSAGE] = str(e)
...
def loadaws_volumes_data(datahex, cloudPayload, cfgDB):
    message = {}
    try:
        cls = get_compute_driver(ComputeProvider.EC2)
        driver = cls(str(cloudPayload[Names.AWS_APIKEY]), str(cloudPayload[Names.AWS_APISECRET]),
        ...
        for node in nodeDetails:
            ...
            temp = str(node.extra[Names.TAGS])
            temp = temp.replace(".", "-")
            temp = temp.replace("'", "\"")
            temp = json.loads(temp)
            extra[Names.TAGS] = temp

            del node.extra[Names.TAGS]
            extra[Names.EXTRA] = node.extra
            extra[Names.ROW_CREATED_TIMESTAMP] = datetime.strptime(now.strftime("%d/%m/%Y %H:%M:%S"),
                                                                   "%d/%m/%Y %H:%M:%S")
            extra[Names.ROW_MODIFIED_TIMESTAMP] = datetime.strptime(now.strftime("%d/%m/%Y %H:%M:%S"),
        ...
        message[Names.RESPONSE_MESSAGE] = "AWS Volume Details Added"
    except Exception as e:
        print("exception: " + str(e))
        message[Names.RESPONSE_MESSAGE] = str(e)
...
def loadaws_volumesSnapshot_data(datahex, cloudPayload, cfgDB):
    message = {}
    try:
        # --------------------------------------------DRIVER DETAILS FOR AWS ACCOUNT
        cls = get_compute_driver(ComputeProvider.EC2)
        ...
        for i in range(countVolums):
            ...
            # print("------INSIDE-",volumeSnapShotList)

        print("-------", volumeSnapShotList)
        DataManager.volumeSnapShot_Data(volumeSnapShotList, cfgDB)
        message[Names.RESPONSE_MESSAGE] = "AWS Volume SnapShot Details Added"
    except Exception as e:
        print("exception: " + str(e))
        message[Names.RESPONSE_MESSAGE] = str(e)
...
def loadaws_pricing_data(datahex, cloudPayload, cfgDB):
    ...
    try:
        ...
        for size in sizes:
            ...
            extra[Names.OWNERID] = datahex
            extra[Names.ID] = datahex_Unique
            extra[Names.UUID] = size.uuid
            extra[Names.ID] = size.id
            extra[Names.EXTRANAME] = size.name
            extra[Names.PRICE] = size.price
            #extra[Names.PRICE] = size.extra
            #extra[Names.INSTANCEFAMILY] = size.extra['instanceFamily']
            #extra[Names.INSTANCETYPE] = size.extra['instanceType']
            #extra[Names.INSTANCESKU] = size.extra['instancesku']
            extra[Names.MEMORY] = size.extra['memory']
            extra[Names.NETWORKPERFORMANCE]= size.extra['networkPerformance']
            extra[Names.NORMALISATIONSIZEFACTOR] = size.extra['normalizationSizeFactor']
            extra[Names.PROCESSORARCHITECTURE] = size.extra['processorArchitecture']
            extra[Names.SERVICECODE] = size.extra['servicecode']
            extra[Names.SERVICENAME] = size.extra['servicename']
            extra[Names.STORAGE] = size.extra['storage']
            extra[Names.VCPU] = size.extra['vcpu']
            #extra[Names.VPCNETWORKINGSUPPORT] = size.extra['vpcnetworkingsupport']
            #sizeList.append(extra)
            DataManager.price_DataStore(extra, cfgDB)


        message[Names.RESPONSE_MESSAGE] = "AWS Price Details Added"
    except Exception as e:
        print("exception: " + str(e))
        message[Names.RESPONSE_MESSAGE] = str(e)
...
def load_cloud_data(datahex, cloudPayload,cfgDB):
    ...
    try:
        ...
        message[Names.RESPONSE_MESSAGE] = "AWS Cloud Account Details Added"
    except Exception as e:
        print("exception: " + str(e))
        message[Names.RESPONSE_MESSAGE] = str(e)
...
def loadaws_sizing_data(datahex, cloudPayload,cfgDB):
    # --------------------------------------------DNS DETAILS FOR AWS ACCOUNT
    ...
    for size in sizes:
        ...
        dataSizing.append(extra)


    print(dataSizing)
    DataManager.sizing_Data(dataSizing,cfgDB)
...
def awsComputation(cloudPayload, cfgDB):
    print("AWS")
    datahex = uuid.uuid4().hex
    jsonData = load_cloud_data(datahex, cloudPayload,cfgDB)#---  CLOUD CONFIGURATION RELATED DATA FOR AWS
    responseMessage = {}
    responseList = []
    if(jsonData[Names.MESSAGE] == Names.NOTEXISTS):
        responseMessage["pollingDetails"] = loadaws_polling_data(datahex, cloudPayload, cfgDB)
        responseMessage["computeDetails"] = loadaws_compute_data(datahex, cloudPayload, cfgDB) #--- COMPUTE CLOUD RELATED DATA FOR AWS
        responseMessage["volumeDetails"] = loadaws_volumes_data(datahex, cloudPayload, cfgDB)
        responseMessage["volumeSnapshotDetails"] = loadaws_volumesSnapshot_data(datahex, cloudPayload, cfgDB)
        responseMessage["locationDetails"] = loadaws_location_data(datahex, cloudPayload, cfgDB)  # --- LOCATION CLOUD RELATED DATA FOR AWS
        responseMessage["subnetDetails"] = loadaws_subnet_data(datahex, cloudPayload, cfgDB)  # --- LOCATION CLOUD RELATED DATA FOR AWS
        responseMessage["networkDetails"] = loadaws_network_data(datahex, cloudPayload, cfgDB)
        responseMessage["imageDetails"] = loadaws_images_data(datahex, cloudPayload, cfgDB)  # --- IMAGE CLOUD RELATED DATA FOR AWS
        responseMessage["accountDetails"] = "Account Details Added"
        #responseMessage["priceDetails"] = loadaws_pricing_data(datahex, cloudPayload, cfgDB)
        responseList.append(responseMessage)
        #loadaws_sizing_data(datahex, cloudPayload, cfgDB)
    else:
        print("Account Details Already ",Names.EXISTS)
        responseMessage["accountDetails"] = "Account Details already exists"
        responseList.append(responseMessage)
    #loadaws_location_data(datahex, cloudPayload, cfgDB)  # --- LOCATION CLOUD RELATED DATA FOR AWS
    #loadaws_images_data(datahex, cloudPayload, cfgDB)#--- IMAGE CLOUD RELATED DATA FOR AWS
    #loadaws_volumes_data(datahex, cloudPayload, cfgDB)#--- VOLUME CLOUD RELATED DATA FOR AWS
    #loadaws_volumesSnapshot_data(datahex, cloudPayload, cfgDB)#--- VOLUME SNAPSHOT CLOUD RELATED DATA FOR AWS
    #loadaws_sizing_data(datahex, cloudPayload, cfgDB)
    print("AWS CLOUD DETAILS CAPTURED")
    return responseList



async def awsComputationCheck(cloudPayload,cfgDB):
    print("AWS CHECK")
    ...
    loadaws_polling_data(datahex, cloudPayload, cfgDB)
    loadaws_compute_data(datahex, cloudPayload, cfgDB)
    loadaws_volumes_data(datahex, cloudPayload, cfgDB)
    loadaws_volumesSnapshot_data(datahex, cloudPayload, cfgDB)
    loadaws_location_data(datahex, cloudPayload, cfgDB)  # --- LOCATION CLOUD RELATED DATA FOR AWS
    loadaws_subnet_data(datahex, cloudPayload, cfgDB)  # --- LOCATION CLOUD RELATED DATA FOR AWS
    loadaws_network_data(datahex, cloudPayload, cfgDB)
    loadaws_images_data(datahex, cloudPayload, cfgDB)  # --- IMAGE CLOUD RELATED DATA FOR AWS
...
if __name__ == '__main__':
    ...
    #logconfig = op.join(resourcepath, "logging.ini")

    #LogManager.initlogger(logconfig)

    datahex = uuid.uuid4().hex
    #app.run(host="localhost", port=5007)
    #loadaws_compute_data(datahex) #--works
    #loadaws_dns_data(datahex)
    #loadaws_volumes_data(datahex) #--works
    #loadaws_volumesSnapshot_data(datahex) #--works
    #loadaws_images_data(datahex,configfile) #---works
    #loadaws_location_data(datahex,configfile) #---works
    #loadaws_sizing_data(datahex,configfile) #------checking for size vs price
    #loadaws_pricing_data(datahex) #-----checking
